...
def _programmatic_mode_resolver(
    worker_ids: tuple[str, ...],
    backend,
) -> StaticWorkerDispatchModeResolver:
    """Inject push mode only from an explicit programmatic backend authority.

    This mirrors the accepted Zero-Relay review composition: the static
    resolver is a narrow adapter over an already-proven invocation surface,
    never an inference from task metadata or a Serena/tunnel runtime.
    """
    if (
        ...
        raise RuntimeActivationError("WORKER_CANDIDATE_AUTHORITY_MISSING")
    if (
        not callable(getattr(backend, "execute", None))
        or getattr(backend, "dispatch_mode", None)
        is not GraphDispatchMode.PROGRAMMATIC_PUSH
    ):
        raise RuntimeActivationError(
            "PROGRAMMATIC_PUSH_BACKEND_AUTHORITY_MISSING"
        )
    return StaticWorkerDispatchModeResolver(
        {
            worker_id.strip(): GraphDispatchMode.PROGRAMMATIC_PUSH
...
_RUNTIME_OWNER_TABLES = frozenset(
    {
        "job_store_meta",
        "job_records",
        "job_events",
...
def _verify_runtime_authority_schema(database_path: str | Path) -> None:
    canonical = Path(database_path).expanduser().resolve(strict=False)
    if not canonical.is_file():
        raise RuntimeActivationError("RUNTIME_SCHEMA_RECONCILE_REQUIRED")
    try:
        ...
        raise RuntimeActivationError(
            "RUNTIME_SCHEMA_RECONCILE_REQUIRED"
        ) from exc
    if not _RUNTIME_OWNER_TABLES.issubset(tables):
        raise RuntimeActivationError("RUNTIME_SCHEMA_RECONCILE_REQUIRED")
...
def _initialize_runtime_authority_stores(
    ...
    """Initialize accepted owner schemas, then verify before dispatch.

    Initializers are independently idempotent and may commit in separate
    connections.  If any stage fails, the canonical DB can therefore contain
    a truthful partial migration.  We never erase or retry it implicitly:
    return a typed reconcile gate.  A later explicit activation may retry the
    same owner initializers and must pass full schema verification before any
    execution is authorized.
    """
    canonical = Path(database_path).expanduser().resolve(strict=False)
    try:
        job_store = SQLiteJobStore(canonical)
        job_store.initialize()
        execution_store = SQLiteExecutionStore(canonical)
        execution_store.initialize()
        lease_store = SQLiteWorkerLeaseStore(canonical)
        _verify_runtime_authority_schema(canonical)
...
def activate_production_runtime(
    *,
    ...
    # schemas may be initialized in the canonical database.  Full owner-table
    # inventory is verified before dispatch; partial initialization is a typed
    # reconciliation state, never an implicit blind retry.
    job_store, execution_store, lease_store = (
        _initialize_runtime_authority_stores(canonical)
    )
    ...
    coordinator = GraphDispatchCoordinator(
        service=durable,
        mode_resolver=_programmatic_mode_resolver(worker_ids, backend),
    )

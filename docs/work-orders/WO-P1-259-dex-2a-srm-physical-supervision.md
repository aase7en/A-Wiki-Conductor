...
- Canonical path identity verification (not ownership): independently recompute the admission-supplied canonical worktree/repo path identity on the execution host using the contract's OS final-path-resolution algorithm (Windows `GetFinalPathNameByHandleW`-equivalent / POSIX `realpath`-equivalent; section 1 of the contract) and compare it before spawn and again before collection binding. Mismatch or canonicalization failure fails closed: no spawn, no collection binding, typed `PROJECT_IDENTITY_FAILED`, quarantine marker on affected evidence. SRM never redefines or owns canonical task/worktree identity. MAY REUSE/EXTEND existing SRM `fs.realpath`-based security/path validation; lexical sunday/path-lock normalization alone is not sufficient evidence for this cross-repo seam.
- Deterministic fake/fault tests: implement the substrate-side halves of every v1-scope DEX scenario in `docs/contracts/fault-injection.md` — all scenarios except the future DEX-3a `NOTIFICATION_ACK_LOSS`, which v1 gates MUST NOT require — (admission response loss, simultaneous dispatch, shim crash before/after spawn, PID reuse, result-before-receipt, receipt response loss, reboot, path alias/junction overlap incl. canonicalization/recomputation mismatch, claim/session loss while child alive, malformed/forged evidence, cross-repo SHA mismatch).
...
- [2026-09-19] DEX-ARCH-1 repair-r1: added canonical path identity verification (not ownership) scope and explicit v1-scope fault-gate wording per independent review (P1, P2-2); no runtime implementation started.

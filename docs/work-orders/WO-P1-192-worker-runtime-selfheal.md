...
- launcher telemetry compatibility has a bounded source gap: all five live launchers use legacy `$RuntimeProcess.WaitForExit()` without `runtime-archive`, `$RuntimeProcess.Refresh()`, or `$RuntimeExitCode`. Current `_harden_start_script_runtime_forensics()` only recognizes the older `Wait-Process -Id $RuntimeProcess.Id` shape. An in-memory reproducer on current main returned `UNCHANGED=True`, `HAS_ARCHIVE=False`, `HAS_REFRESH=False`, `HAS_EXIT_VAR=False`. Unless a different supported existing-instance upgrade path is proven, the smallest child source repair is `src/a_conductor/instance_create.py` + `tests/test_instance_create.py`: RED the live method-wait variant, harden it idempotently while preserving credential/preflight logic, and independently review before use.

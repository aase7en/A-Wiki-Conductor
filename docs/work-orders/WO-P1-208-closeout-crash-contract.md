...
### Finalization S1–S9 checkpoint — 2026-09-12 (GLM)

All four review findings closed through the bundle only; zero src/tests delta vs source base 02d39cb across the entire finalization (verified). Delta vs preparation HEAD d470880 = 11 files, ALL within the authorized scope (bundle + WO checkpoints).
- S1/N1: seed_probe binds executable bytes to the canonical fence BEFORE import (raw or CRLF→LF-normalized match; mismatch rejected, sentinel proves accepted code ran); `lab_common.verify_manifest` fail-closed preflight (bundle hashes + 4 pinned source blobs; manifest excludes itself; explicit CRLF→LF rule so any checkout verifies cross-platform; trust root = pinned candidate SHA, no self-authentication claim).
- S2/S3/N2: c07 transition CLOSED over the four-axis schema (I7), first execution counts ONE effect, duplicates observable, `next_state` validated not discarded; blocked-job EXECUTE_ONCE (I1) and stale-owner COMPLETE (I1c) mutants now detected (9 mutants total); COMPLETE-vs-COMPLETE_UNMODELED distinction documented; six bounded traces with truthful adversarial-unbacked-completion-refused.
- S4/N3: positives AND negative controls route through the ONE shared `validate_cut_observation` with per-reason rejection (10 controls incl. wrong-exit/marker/effects/refs/state through the validator); combined wrong-exit-99 mutant now fails the driver (exit 1, reasons `exit:10!=99`).
- S5: REAL finite helper timeout (hang-past-timeout child killed at the bound, nonzero sentinel) + bounded-sibling reap + early-first-child-fail + missing-output controls; owned helpers only.
- S6/N4: c06 report now agrees with the journal (port_called vs released; checkpoint PRESENT is the observed fact); REQUIRED reload experiment added — reopen store, retain truthful ACTIVE lease, execute again → RECOVERY_REQUIRED / LEASE_RELEASE_CONTRADICTION, exactly 1 release call, job REVIEW_PENDING; synthetic RELEASED positive control kept distinct and labelled; F2 stays a port-contract question.
- S7: differential/metamorphic batch in replay_suite (entry-order permutation, c07 replay determinism on typed verdicts, corrupt-summary not success, idempotent suite rerun).
- S8: NEW replay_suite.py (one documented batch: preflight → 4 entrypoints → differential; per-exit preserved; run-N dirs never delete evidence; cold-start git shim for the immutable fence's source_sha observation with a documented note) + test_lab_contract.py (24 tests: validator reasons, preflight fail-closed classes, seed binding, c07 closure through the real module, aggregation discipline); README batch/pytest commands with Windows/POSIX examples; manifest regenerated LAST in a single final pass.
- Cold-start native Windows proof: fresh `git archive` export of the exact candidate, no prior ignored runs → baseline 94/94, contract 24/24, batch exit 0 (preflight + 4 entries + 4 diffs), second batch replay exit 0, and the export tree shows ZERO generated tracked changes after both runs.
- MAC_REPLAY_PENDING: native Mac replay remains for Astra (bundle is cross-platform by the explicit LF rule; never labeled simulated as native).
- Stop state: COMPLETE_FOR_INDEPENDENT_REVIEW.

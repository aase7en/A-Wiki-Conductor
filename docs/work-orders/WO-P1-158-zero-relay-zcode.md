...
## Q29 authority-complete assembly + Q30 truth audit checkpoint — 2026-09-06

- **Assembly now truthful:** `verify_execution_context` is EXECUTED inside `assemble_zcode_execution` (git/worktree/HEAD/dirty gates run before anything else — drift/mismatch/dirty ⇒ typed `ZCODE_HEAD_DRIFT`/`ZCODE_BRANCH_MISMATCH`/`ZCODE_WORKTREE_DIRTY` with zero child); the dead `lease_broker` parameter was removed and replaced with `lease_evidence` / `admission_evidence` — the assembly CONSUMES accepted records from the higher-level coordinator (missing/false ⇒ `ZCODE_LEASE_ADMISSION_MISSING` / `ZCODE_PROVIDER_ADMISSION_MISSING`, zero spawn) and never re-acquires or schedules leases/admissions itself (source-scan test: no broker construction, no acquire(), no second store, no threading); packet intake is now confined to `trusted_root=repo_root` (escape ⇒ typed reject at intake AND at the pre-send re-read).
- **Recovery consumer composed into production:** `ZCodeBackendAdapter.recover(execution_id)` is a real production caller of `read_child_identity_from_run_dir` + `reconcile_zcode_child` under the existing recovery authority (evidence-only; attach vs RECOVERY_REQUIRED incl. PID-reuse) — proven live by restart composition tests.
- **Adapter lifecycle constrained:** source-scan proofs — the adapter contains no Popen/subprocess.run/taskkill/kill/terminate; the child exists only through the repository transport factory seam; `launch()` dispatch on the closed helper kind is LIVE (not dead); generic `_build_owned_spec` path unchanged.
- **Q30 truth audit:** NEW `tests/test_zcode_composition_truth.py` (4/4): recovery composed attach + PID-reuse; every declared assembly gate is executed (source-proof per gate); no dead helper builders / no transport-spawn bypass / no kill ladder.
- Declared remaining (GPT1 P2, next slice): the bounded protocol turn still executes synchronously inside adapter `launch()`; moving the long operation into the supervised helper process with coordinator-timeout bounding remains open.
- Verification: full battery (composition-truth + assembly 17 + durable 9 + runner 30 + helper-execution 13 + child-recovery 11 + phase-C + helper-kind + supervised execution/runner/coordinator/child + dedup + artifacts + recovery-reconciliation + worker-lease) = **237/237**; compileall/diff-check/UTF-8/secret PASS.

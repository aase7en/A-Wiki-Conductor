...
Top-right: **Guide** opens this guide (language follows the app language). **Settings** has: **supervised mode**, **stop-all-connectors-on-exit**, **Language**, and a read-only **MODELS & AGENTS** status panel. The provider panel refreshes asynchronously and shows configured/ready/authorization, health, trust/egress, generation, model/harness, quota, safe provenance, and observation age without displaying endpoint or credential values. **Refresh** is manual in this first slice. Select a provider to **Edit**, **Disable/Enable**, or **Test** it. Edit is generation-bound and never shows the current credential or endpoint; a blank credential-reference field keeps the existing reference. Disable/Enable is refused while active or unreconciled admissions still use that provider. Test runs in the background through the accepted runtime probe; routes with no probe authority show **UNSUPPORTED**, not a fake pass/fail. The window title always carries the version, e.g. `A-Sunday Conductor v0.7.0`.

Press **Evidence...** for a read-only detail view of the selected provider. It keeps `SELECTION_REASON=UNKNOWN` and `FALLBACK_REASON=NOT_EVALUATED` unless future accepted authority persists something stronger, separates current operator evidence and declared capabilities from up to 10 newest admissions, and never treats admission lifecycle as execution success/failure. When an explicit Graph Monitor `graph_id` + `graph_run_id` already exists, a node is shown only when its derived job ID exactly equals the admission `execution_id`; otherwise the detail says `NO_READABLE_GRAPH_EVIDENCE`. No latest-run inference, endpoint value, or credential value is exposed.

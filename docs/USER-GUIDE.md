...
เวอร์ชันล่าสุด: 2026-08-25 (v0.6.0) · ระบบปฏิบัติการที่รองรับ: **Windows 10/11** (ฟีเจอร์เต็ม) · Python 3.11+ (ถ้าติดตั้งแบบนักพัฒนา)
...
2. ในโปรแกรม กด **Set Tunnel ID** ที่ connector แต่ละแถวเพื่อดู/ใส่ ID แล้วเทียบว่าตรงกับตัวไหน (ตารางโชว์แค่ `Y/-` ว่ามี ID หรือยัง ยังไม่โชว์ค่า ID เอง)
3. เมื่อเจอตัวที่ตรง: อ่านคอลัมน์ **PROJECT** และ **PORT** ของแถวนั้น = แชทนี้กำลังทำงานกับโปรเจกต์นั้น ผ่านพอร์ตนั้น
4. ในตาราง WORKERS คอลัมน์ **CONNECTOR (?)**: worker ใดที่ถูก assign โปรเจกต์เดียวกับ connector ตัวนั้น (path ตรงกันเป๊ะ) จะโชว์ชื่อ connector; ขีด `-` = ยังไม่มี connector ไหนผูกกับโปรเจกต์ของ worker นั้น (ไม่ใช่ error)

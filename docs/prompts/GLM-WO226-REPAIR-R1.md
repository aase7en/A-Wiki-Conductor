...
- **AF4 / P1 trusted provider policy:** before any acquisition/model effect, consume the existing provider-policy authority for the trusted C0 task (including network/security policy). Canonical denial such as `TASK_NETWORK_DENIED` must fail closed. Do not invent a second policy engine. **Direct review also requires the complete provider-authority triple to exist**: `provider_endpoint`, `provider_security`, and positive `expected_configuration_generation` are mandatory for this boundary. An all-None or otherwise missing authority triple must fail closed before lease/admission/model effect; never substitute the ambient/current snapshot as authority for an authority-less task. Add a RED proving all-None authority yields zero resource acquisition and zero model effects; preserve the full-authority positive path.

...
Cross-repo dependency RELEASED: A-Wiki Review Bridge exact head `b04761d5...` passed independent GLM rereview P0/P1/P2=0, merged as `588a9072...`, and post-main Core CI `33704270521` is SUCCESS. WO147 now owns the thin A-Conductor mailbox adapter and must reuse `agent-mailbox/v1` plus the approved A-Wiki `conductor review` CLI; no parallel ReviewBus/lifecycle authority.
...
### Session checkpoint — 2026-09-04 12:06 +07 — WO-P1-156 / Fast-Roadmap parallel recovery

- Primary repository: `A:\GitHub\A-Wiki-Conductor`; authoritative remote `origin/main@68079e3d00047ca9432f0aefe3ad667f892614d0`. Protected root worktree remains stale/dirty and is not a mutation surface.
- GPT docs/continuity lane: worktree `A:\GitHub\_worktrees\A-Wiki-Conductor-wo156-defect-memory`, branch `docs/wo-p1-156-worker-tunnel-incident`, PR #209. Scope is only durable incident/defect/continuity evidence; no WO156 source mutation.
- GLM implementation lane: worktree `A:\GitHub\_worktrees\A-Wiki-Conductor-wo156-worker-resilience`, branch `fix/wo-p1-156-worker-resilience`. Last independently observed head before the user's latest GLM run: `b46f7dd1109bcee9e915083be57e7c92f38ba923`. GPT has NOT accepted that head because it still contains a duplicate recovery-store/backoff/circuit authority. User has now dispatched the superseding single-authority WO156 prompt; reconcile actual head/result before accepting anything.
- Confirmed Worker failure chain: `start.ps1 -> tunnel-client.exe -> serena.exe`. A tunnel-client exit removes its Serena child; Worker 4 reproduced repeated `TUNNEL_START_FAILED` followed by Serena stdout-flush `OSError: [Errno 22]`.
- CPU/RAM are not supported as the primary cause: sampled CPU ~3.9%, ~4.21 GiB/15.89 GiB RAM free, and no inspected Windows resource-exhaustion/WHEA critical events.
- ESET is a possible upstream trigger only. ESET Service/Firewall Helper/Inspect Connector are active, but no inspected Windows/ESET evidence directly ties ESET to `tunnel-client.exe` termination. Do not disable ESET globally or create an exclusion without a bounded A/B plan and evidence.
- Critical deployment gap: installed `A-Sunday Conductor LATEST.exe` is dated 2026-08-26 (SHA-256 `96ADA142A2DDB7293C17F140B4CFC4B7D3288599938AD207A57D0A05B40CA201`), while bounded connector recovery source was added on 2026-08-28. Live DB lacks `instance_recovery`; copied-live-DB migration adds it, preserves existing table row counts, and passes `PRAGMA integrity_check=ok`. Live DB was not mutated.
- Critical tunnel-client gap: all live Sunday Workers still share `tunnel-client 0.0.11+8d55683eeef80bc5e360d95abf4692454fafc615` at `C:\\AI\\dwb-serena-tunnel-starter\\tunnel-client\\tunnel-client.exe` (SHA-256 `7D3C7D492CE84B52835E11865A835A8A5BCD4A669DEE84E169AA11B314DC952A`). WO-P1-097 already classified 0.0.11 below the safe floor and requires `>=0.0.12`; its live-fleet upgrade step was intentionally deferred and remains unresolved. This is now the strongest initiating-trigger hypothesis, ahead of ESET. A temp-only run through the existing checksum-verified installer resolved current candidate `0.0.14`, SHA-256 `FCC85A69EC0AD82518E4F8964F60C45E31787957782A0FC9C1B0C44E82D61B9B`, then removed the temp directory; live binary remains untouched.
- Existing recovery authority is already wired in current source: `ConnectorRecoveryCoordinator + ConnectorRecoveryRecord + SQLiteSerenaConfigStore + LocalInstanceOrchestrator`; `desktop_control.instance_states_cancellable()` calls `reconcile_instance_recovery()`. Baseline recovery suites on `origin/main`: 52/52 PASS.
- Architecture decision: exactly ONE durable recovery/retry authority. WO156 must REUSE/WRAP/EXTEND the existing connector-recovery path, not preserve `WorkerRecoveryStore` / duplicate backoff/circuit/restart-budget authorities when responsibilities overlap.
- Incident memory is tracked in `DEFECT_LESSONS.md` lesson #52 and `docs/incidents/2026-09-04-worker-tunnel-teardown.md`. Incident stays OPEN until an installed/frozen build self-recovers an intentionally interrupted isolated Worker without manual relay, duplicate processes, broad kill, or blind task replay. Live execution steps/rollback are pinned in `docs/runbooks/wo156-live-tunnel-upgrade-and-chaos.md`.
- Current Worker observation during this session: Worker1 was manually recovered from its exact durable launch spec after proving port/process absence; Worker2/3/5 remained live; Worker4 had reproduced the tunnel teardown and was not used for mutation. Always re-probe before reuse because this state is ephemeral.
- PR #204 WO152 authoritative branch was fast-forwarded to `9c90c873493b657cced86652c09c0e19920d99c8`; related AiPASS discovery suite 152/152 PASS and GitHub CI is green on that exact head. Do not merge out of frozen roadmap order.
- PR #208 WO154 remains at `0fd540c622d4539a2e809b8a441661896179f2ad` with green CI but independent exact-SHA review gate still unresolved. Do not count timed-out ZCode reviews as approval.
- WO155/ZRA last independently reviewed head: `653eba9f348293008bdef1a8a4716a23698d8f62`; focused tests 50/50 PASS, but GPT found R3 blockers: session-event identity, process-reap evidence, task-packet TOCTOU, streaming output bound, and overclaimed ZRA-1/ZRA-3 authority. Treat live pilot only as transport/harness proof until repaired and re-reviewed.
- PR #202 ODP-1 remains at `8cd1a5007a60966b003597ac4cf31e221c10be55`. Its worktree contains an ownership-unknown dirty patch that makes `tests/test_orchestration_packet.py` 54/54 PASS; do not commit/overwrite it until ownership is reconciled or reproduced in a fresh claimed lane.
- Exact next safe GPT actions while GLM works:
  1. keep PR #209 exact-head CI/review evidence current and merge only when exact-head gates pass;
  2. continue PR #208 exact-SHA review gate without blind replay of timed-out executions;
  3. prepare installed WO156 migration + isolated chaos acceptance using copied DB and sacrificial/idle Worker only;
  4. after GLM reports done, reconcile actual WO156 HEAD/diff/tests against this checkpoint, then independent GPT R3 review;
  5. only after accepted source candidate: build/install verified candidate and run exact-PID logical-Worker self-heal chaos cycle.
- Forbidden: broad-kill Python/Node/ZCode/Serena/tunnel-client; live-DB mutation before reviewed migration gate; global ESET disable; blind replay after ambiguous task outcome; taking dirty/claimed worktrees; merging claims solely because an agent says DONE.
- GLM WO156 superseding candidate observed at `55de87f04dde7b0682026ef2c45ce17096de66db`: duplicate `WorkerRecoveryStore` / `BackoffSchedule` / `WorkerCircuit` / `RestartBudget` authorities are removed; policy module reduced to classification/disposition/availability/process-identity/fleet-correlation seams. Required regression battery independently rerun by GPT: 145/145 PASS + compileall + diff-check.
- GPT R3 review blocker on `55de87f`: `worker_resilience.py` has no production caller outside itself/tests for `classify_worker`, `coordinator_disposition`, or `worker_available_for_work`. Current chaos script calls `ConnectorRecoveryCoordinator` directly with `_MemStore` and a manual start seam; it proves core recovery/process behavior but NOT installed-app automatic detection/recovery. Candidate is therefore NOT ACCEPTED yet.
- Next WO156 requirement: either wire the minimal policy into an existing production observation/recovery seam without creating another monitor/authority, or remove dead policy that is not required for the incident. Installed close gate remains production `DesktopControl`/real store + reviewed build + isolated exact-PID chaos, not direct coordinator invocation.
- W1 tunnel-client canary preflight (2026-09-04 ~12:45 +07): checksum-verified candidate `0.0.14` installed only to `C:\\AI\\dwb-serena-tunnel-starter\\canary\\0.0.14\\tunnel-client\\tunnel-client.exe`, SHA-256 `FCC85A69EC0AD82518E4F8964F60C45E31787957782A0FC9C1B0C44E82D61B9B`. Candidate doctor loaded W1 profile and command successfully; its only failure was expected `health_listener` bind conflict because old W1 still owned port 18011. Shared live `0.0.11` binary remains untouched.
- W1 exact pre-canary process identity: wrapper PowerShell PID 30052 -> old shared tunnel-client PID 15056 -> Serena PID 21160; port 18011 READY; instance path points to shared 0.0.11. Planned canary is W1-only with backup/rollback of `instance.ps1`; W2/W3/W5 are forbidden scope and remain on shared binary. This is an operational A/B, not a fleet upgrade.
- W1 canary switch COMPLETE: exact old W1 tunnel PID 15056 was stopped by the instance-scoped guarded stop script; `instance.ps1` backup `instance.ps1.wo156-canary-20260904-124614.bak` SHA-256 `0B95073B0AD3179FFE66FACAD777A62313DB0C3D0FB8626A60270775550AD5AC` was created before mutation. W1 alone now points to checksum-verified canary `0.0.14` at `C:\\AI\\dwb-serena-tunnel-starter\\canary\\0.0.14\\tunnel-client\\tunnel-client.exe`.
- W1 0.0.14 post-switch evidence: doctor PASS after port drain; new tunnel PID 11956, exactly one W1 tunnel process, Serena child PID 2640, `/readyz` PASS, and ChatGPT remote MCP `get_current_config` PASS. W2/W3/W5 shared 0.0.11 processes were untouched. This proves 0.0.14 profile/runtime/remote compatibility for W1; it does NOT by itself prove 0.0.11 caused every prior exit. Keep canary active for stability evidence; rollback path is the recorded backup.
- Clean `origin/main@68079e3` recovery-build lane: detached worktree `A:\\GitHub\\_worktrees\\A-Wiki-Conductor-wo156-main-recovery-build`, isolated Python 3.13.13 env via uv, exact CI `PyInstaller==6.22.2`. Recovery baseline rerun in this isolated env: 52/52 PASS. Portable build `A-Sunday Conductor.exe` size 31,329,537, SHA-256 `26817C8EB8F2BF8AA8C5898C5640C033CBC5CE4809D9E276FF05251028CE4526`; frozen smoke EXIT=0 with isolated DB; archive contains `a_conductor.connector_recovery`, `serena_config_store`, `desktop_control`, `local_instances`. Setup build size 38,288,452, SHA-256 `261A9D87F0B34ECC046981867788D2003504E3333CE2CD84B45D4694ACC509D2`; required setup payload archive entries PASS and heavy UI/OpenGL modules are absent from Setup archive as expected. No install/live DB mutation has occurred.
- W1-vs-W4 live A/B is active: W1 canary runs tunnel-client 0.0.14 (PID 11956 -> Serena 2640, READY, remote MCP PASS) while W4 control remains shared 0.0.11 (PID 24088 -> one Serena child, READY). W4 has prior spontaneous `TUNNEL_START_FAILED` at 10:21:31 and 11:02:36; W1 has no post-canary failure observed so far. Keep W4 on 0.0.11 temporarily as control unless its availability becomes operationally unsafe.

### GPT-B / PR #209 checkpoint - 2026-09-04 20:00 +07

- Claim remains GPT-B docs/operational-evidence lane on PR #209. No WO156/WO155/WO152/ODP source mutation authority.
- Remote refs rechecked before mutation: main@68079e3, PR208 branch 0fd540c, PR209 branch cad47bc, WO156 branch 719f30e, coordination branch e3549a8.
- W4 control recurrence at 18:31 is now exact-log evidence: MCP connection TTL reached -> stdio write file already closed -> tunnel shutdown -> Serena exit 120 / stdout-flush OSError 22 -> wrapper TUNNEL_START_FAILED.
- At 19:53 W1 canary 0.0.14 remained LISTEN on PID 11956 from 12:46:55; W4 had no listener. W2/W3/W5 legacy 0.0.11 controls remained LISTEN. Classification: trigger-dependent legacy failure family, not fixed-uptime failure.
- ESET remains unconfirmed; the 18:29-18:34 Windows Application/System window contained no matching ESET/AV/OS Error or Warning tied to the exit.
- WO156 actual pushed candidate is now 719f30e377be8302a309fd9d5f906f7110a855ec, clean. GPT independently reran the required eight-file battery as 131/131 PASS plus compileall, diff-check, and strict UTF-8 PASS.
- WO156 is nevertheless NOT ACCEPTED. GPT R3 architecture correction: the incident contract says ambiguous task state blocks replay/AVAILABLE, but the exact Worker process may still self-heal first. Candidate 719f30e instead treats ownership_unsafe/task_state_unknown/quarantined as connector-recovery holds. This conflates process self-heal authority with dispatch/reassignment/task-replay authority.
- DesktopControlService.open() does not assemble recovery_hold_provider, and read-only inspection of the live Control Center DB found no lease tables or Serena worker-config rows to supply such a mapping. Do not invent a second ownership/task authority inside DesktopControlService. Keep unexpected STOPPED self-heal in ConnectorRecoveryCoordinator; keep task/ownership/dirty-state gating in the existing lease/dispatch/AVAILABLE path. If the hold seam is retained for an explicit accepted process-quarantine authority, malformed flags must fail closed; unexpected key currently raises TypeError and non-bool raises ValueError.
- The copied-live-DB test is a valuable copied-DB integration/method-path proof, but it constructs DesktopControlService directly with null/fake seams and later injects the hold provider. It is not full DesktopControlService.open() production-composition proof.
- WO156 COLLAB.md delta is stale/hotspot overlap with PR208 (text still references the earlier 33-test candidate); integrator should remove/reconcile that shared-hotspot change before acceptance rather than merge stale claim text.
- WO155 653eba9 preliminary GPT audit found architecture/resource blockers in addition to earlier findings: raw subprocess.Popen lifecycle conflicts with DEFECT_LESSONS #21 / accepted supervised-process authority; process_reaped may be serialized true despite ambiguous cleanup; stdout queue and streamed text accumulation are unbounded before output-limit enforcement; repair context can be truncated out at 4096 chars; the live pilot synthesizes provider readiness/identity and bypasses production admission/policy/secret-resolution authority. Treat it as LIVE_CALLABILITY_PROOF only until repaired and re-reviewed.

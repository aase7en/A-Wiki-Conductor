...
- WO156 is nevertheless NOT ACCEPTED. P1 blocker: DesktopControlService.open() does not assemble a recovery_hold_provider; ownership/task/quarantine holds exist only through injected constructor seams in tests, so the production composition does not prove the claimed hold gate.
- WO156 second P1: _recovery_hold() catches provider-call exceptions and non-dict values, but exceptions raised while validating the returned dict escape. Reproduced cases: unexpected key -> TypeError; non-bool flag -> ValueError. Unknown/malformed hold data must fail closed instead of breaking refresh.

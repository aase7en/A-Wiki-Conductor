...
- Incident memory is tracked in `DEFECT_LESSONS.md` lesson #6 and `docs/incidents/2026-09-04-worker-tunnel-teardown.md`. Incident stays OPEN until an installed/frozen build self-recovers an intentionally interrupted isolated Worker without manual relay, duplicate processes, broad kill, or blind task replay. Live execution steps/rollback are pinned in `docs/runbooks/wo156-live-tunnel-upgrade-and-chaos.md`.

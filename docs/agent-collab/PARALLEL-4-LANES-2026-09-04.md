...
- GPT-A WO152 preflight found PR #204 is NOT merge-ready despite green CI: `9c90c87` still differs from merge-base in 6 files, while its own final-scope contract requires exactly 3; staging tests `test_aipass_discovery_boundary_hardening2.py`, `...hardening3.py`, and `...post_ci_boundaries.py` remain tracked and must be consolidated into canonical `tests/test_aipass_discovery.py` before a new reviewed candidate. PR #204 body is also stale at `d5dae16`. Do not merge #204 on green CI alone.
...
- Stronger frozen-binary migration proof used SQLite backup API to snapshot the live DB, then ran the frozen Portable against the snapshot only: smoke exit 0, before/after integrity `ok`, old table row counts preserved, old table schemas preserved, and only the expected new tables were added (`instance_recovery`, `provider_admissions`, `provider_configurations`, `provider_endpoints`, `provider_observations`). The snapshot/temp files were removed; live DB was not mutated.
- Setup payload identity proof: PyInstaller archive extraction of `payload\\A-Sunday Conductor.exe` from Setup SHA `261A9D87...` produced SHA-256 `26817C8EB8F2BF8AA8C5898C5640C033CBC5CE4809D9E276FF05251028CE4526` and size 31,329,537 bytes, exactly matching the Portable artifact already smoke/migration-tested. Therefore the Setup embeds the same tested Portable, not a different binary.
...
## GPT-A checkpoint — 2026-09-04 ~22:20 +07

- PR #208 independent GLM exact-SHA review of `0fd540c622d4539a2e809b8a441661896179f2ad` completed `CHANGES_REQUIRED`, P0=0/P1=0/P2=4. GPT-A accepted the review and opened `GPT1-WO154-REPAIR-001`.
- PR #208 repair candidate was frozen and non-force fast-forwarded to `5124ed18409d71df7b98e4455f0a4a2df6428695` on the same PR branch. Final diff remains exactly the declared 12 docs files. Repairs: scope chronology, final 12-file verification truth, ZRA-4 broad-ODP resume boundary, WO153 release chronology, ZRA-0 pre-proven-vs-formal-acceptance distinction, and capability-vs-automatic-route freshness.
- Local PR #208 repair verification at `5124ed1`: `git diff --check` PASS; strict UTF-8/no-U+FFFD across all 12 PASS; added-line secret-shape scan 0 hits; `tests/test_operator_protocol.py` 37/37 PASS. Exact-head CI run `33888881360` is in progress. Required next gate: focused independent READ_ONLY rereview of `5124ed1`, then GPT-A merge adjudication.
- WO156 / PR #218 is frozen at `b8a145b9a6db4ca7e27fb105077094ce7ea3dd42`. GPT-A source/evidence rereview PASS: exact 2-file diff, no `src/a_conductor` delta, independent recovery battery 124/124, compileall/diff/UTF-8 PASS. macOS/Ubuntu exact-head checks are green; Windows main job remains in progress at this checkpoint. Installed live canary/chaos is still a separate operational gate.
- PR #209 advanced to `1b3f060941e5f488340e38ef49139d284da86371`; GPT-A docs/runbook audit PASS and exact-head CI `33887327084` is SUCCESS across Windows/Ubuntu/macOS. Keep unmerged until PR #208 merge-order gate clears; after main changes, recheck merge-base/content identity before merge.
- ZRA-0 pre-acceptance rehearsal on detached clean `main@68079e3` corrected one shaping assumption: `DesktopControlService.open()` intentionally seeds 3 default workers when registry is empty. Formal acceptance must preserve existing data except documented canonical bootstrap mutations. A valid non-empty registry rehearsal preserved workers/projects/assignments/unrelated sentinel rows exactly, added provider tables with zero fabricated provider rows, and returned `PRAGMA integrity_check=ok`. This is prerequisite evidence only; formal ZRA-0 node acceptance remains dependency-gated after PR #208.
- Frozen order remains: `PR #208 -> WO156 reliability close gate -> ZRA-0..4 -> WO152/#204 -> ODP-1..8 -> ZRA-5 -> ODP-9 -> WO096 -> v0.7 stable -> cleanup/SSoT`.

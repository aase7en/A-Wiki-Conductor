...
- Exact repros retained in PR #314 / Issue #214: (1) timeout retains resources -> durable runtime becomes `FAILED` -> persisted admission generation is changed away from plan generation -> replay returns `RECOVERY_REQUIRED / EQUIVALENT_TERMINAL_NOT_USABLE` but still releases admission and lease because `_terminal_unusable_cleanup()` omits expected generation; (2) direct-review task with `provider_endpoint/provider_security/expected_configuration_generation = None/None/None` is accepted and produces `EXECUTED`, one model effect and a usable handoff, despite the WO226/ZRA-1 authority-binding contract.
- Repair packet: remote branch `origin/docs/wo-p1-226-repair-r1@a5ea7938a7a273b2a9554c5e434b2a4673e25229`; durable pointer file `docs/prompts/GLM-WO226-REPAIR-R1.md`. It preserves prior admission batch/generation repair plus Astra AF1-AF4, adds terminal-unusable wrong/unknown-generation REDs, and makes direct-review provider endpoint/security/positive-generation authority mandatory. Expected mutable source scope remains only `src/a_conductor/zero_relay_review_execution.py` + `tests/test_zero_relay_review_execution.py`; scope expansion requires GPT adjudication first.

...
- PR #104 / `feat/ge-6-scheduler`: GE-6 owner lane (GLM 5.3 MAX); re-reconciled with `origin/main @ ab28dc7`; round-2 blockers (topo order, stable worker pick, mutating identity, eligibility seam) GPT-verified fixed with green exact-head CI; final round adds authoritative explicit worker binding (BINDING kind, never falls back) + human-approval pending and stable typed `BlockedReasonKind` per ADR §7; full graph suite 114 passed locally; awaiting final exact-head CI + GPT re-audit. No scheduler mutation by non-Graph lanes.

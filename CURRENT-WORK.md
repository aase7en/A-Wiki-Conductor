...
- **WO226 is the active critical path and is NOT accepted.** PR #314 latest frozen candidate at this checkpoint is `cc398d6b1bff329d7145d0a98662add108b423ab`; its exact-head CI is green, but GPT acceptance-prep reproduced two remaining R3 trust blockers: (1) terminal-unusable cleanup (`FAILED/PARTIAL/CANCELLED`) can release admission+lease while persisted provider admission generation does not match the plan because `_terminal_unusable_cleanup()` passes no expected generation; (2) a direct-review `ParallelReadyTask` with provider endpoint/security/generation all absent is silently upgraded from the current snapshot and can execute one model effect + produce a handoff. Therefore `cc398d6...` is `CHANGES_REQUIRED / DO_NOT_MERGE`.
- The current WO226 repair authority is the same bounded two-file source scope. Durable repair packet is on `origin/docs/wo-p1-226-repair-r1@a5ea7938a7a273b2a9554c5e434b2a4673e25229`, file `docs/prompts/GLM-WO226-REPAIR-R1.md`. It includes admission batch/generation binding, Astra AF1-AF4, terminal-unusable generation binding, and mandatory direct-review provider endpoint/security/positive-generation authority. No schema/store/lock/scheduler/C1 authority expansion is released.
...
One next safe action: re-pin Issue #214 + PR #314 first. If `cc398d6...` is still current, continue the SAME WO226 repair owner/scope using `origin/docs/wo-p1-226-repair-r1:docs/prompts/GLM-WO226-REPAIR-R1.md`; require RED-first terminal-unusable wrong/unknown-generation cases **and** missing-provider-authority zero-effect denial, freeze one new exact SHA, then independent R3 rereview + CI + GPT acceptance before any merge or WO223 release.

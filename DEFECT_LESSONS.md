...
**Fix:** ambiguous provisioning residue now remains consuming under stale reconciliation whether a worker id is bound or not. A bound worker requires typed decommission evidence; an unbound `ACTIVE`/`RECOVERY_REQUIRED` row requires typed runtime-absence evidence because the worker may have been created before its identity was durably persisted. The boolean escape was removed. Generic `RECOVERY_REQUIRED -> RELEASED` and `CAPACITY -> RELEASED` are forbidden. Explicit `release_unstarted()` remains the only release path when the caller knows provisioning never started.

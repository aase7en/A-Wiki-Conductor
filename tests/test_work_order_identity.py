...
- Files declaring ``Identity schema: ISSUE_NUMBER_V1`` must carry an
  ``Issue: #N`` line whose N equals the filename numeric id.
- GitHub-backed work orders with numeric id >= 381 that declare an Issue
  line must keep issue number == numeric id even when the marker line is
...
IDENTITY_MARKER = "Identity schema: ISSUE_NUMBER_V1"
    ...
    for filename, content in sorted(files):
        ...
        if IDENTITY_MARKER in content:
            if not token.isdigit():
                ...
                violations.append(
                    f"{filename}: {IDENTITY_MARKER} requires an 'Issue: #N' line"
                )
            elif any(n != int(token) for n in issues):
                violations.append(
                    f"{filename}: {IDENTITY_MARKER} requires Issue number "
                    f"== {int(token)}, found {sorted(set(issues))}"
                )
        ...
        if github_n is not None and github_n >= GITHUB_BACKED_MIN_N and issues:
            if any(n != github_n for n in issues):
                violations.append(
                    f"{filename}: GitHub-backed work-order id >= {GITHUB_BACKED_MIN_N} "
                    f"requires Issue number == {github_n}, found {sorted(set(issues))}"
                )
    return violations
...
def test_marker_issue_mismatch_is_detected(tmp_path: Path) -> None:
    ...
    assert IDENTITY_MARKER in violations[0]
    assert "requires Issue number == 414" in violations[0]
    assert "GitHub-backed" in violations[1]
...
def test_marker_requires_issue_line(tmp_path: Path) -> None:
    ...
    violations = check_work_order_identities(iter_directory_work_orders(tmp_path))
    assert len(violations) == 1
    assert "requires an 'Issue: #N' line" in violations[0]

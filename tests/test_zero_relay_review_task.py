...
def test_g1_exact_control_digest_is_stable_and_shaped() -> None:
    refs = deterministic_review_refs(_identity())
    assert refs.digest == hashlib.sha256(canonical_review_bytes(_identity())).hexdigest()
...
def test_g1_repeat_input_same_identity() -> None:
    assert canonical_review_bytes(_identity()) == canonical_review_bytes(_identity())
    assert deterministic_review_refs(_identity()) == deterministic_review_refs(_identity())
...
def test_g1_each_field_changes_identity(field: str, value) -> None:
    assert deterministic_review_refs(_identity(**{field: value})) != deterministic_review_refs(_identity())


def test_g1_canonical_bytes_use_fixed_keys_and_json_shape() -> None:
    doc = json.loads(canonical_review_bytes(_identity()).decode("utf-8"))
    assert doc == {
...
def test_g1_unicode_refs_hash_exact_utf8_bytes() -> None:
    ...
    raw = canonical_review_bytes(identity)
...
def test_g1_raw_path_input_cannot_affect_deterministic_shape() -> None:
    refs = deterministic_review_refs(_identity(task_contract_ref="../evil/../wo"))
...
def test_g2_render_binds_every_identity_field_exactly_once() -> None:
    text = render_review_task_markdown(_identity())
    for value in (TASK_REF, TASK_SHA, RESULT_REF, RESULT_SHA, "attempt-1", AUTHOR):
...
def test_g2_render_is_deterministic_no_env_or_time() -> None:
    one = render_review_task_markdown(_identity())
    two = render_review_task_markdown(_identity())
    assert one == two
    import os
...
def _materialized() -> MaterializedReviewTask:
    fs = _FakeNative()
    return materialize_review_task(fs, _identity()), fs


def test_g3_create_persists_exact_bytes_and_sha() -> None:
    task, fs = _materialized()
    content = render_review_task_markdown(_identity())
...
def test_g3_same_bytes_second_call_reuses_packet() -> None:
    ...
    second = materialize_review_task(fs, _identity())
...
def test_g4_positive_read_only_route_binds_every_fact() -> None:
    ...
    route = bind_direct_review_route(_route_task(task), review=task, author=_identity())
...
def test_g4_project_mutation_rejected() -> None:
    ...
    with pytest.raises(ZeroRelayReviewTaskError) as exc:
        bind_direct_review_route(mutated, review=task, author=_identity())
...
def test_g4_author_equals_reviewer_execution_rejected() -> None:
    ...
    fs = _FakeNative()
    task = materialize_review_task(fs, _identity(author_execution_id=derived))
    with pytest.raises(ZeroRelayReviewTaskError) as exc:
        bind_direct_review_route(_route_task(task), review=task,
                                 author=_identity(author_execution_id=derived))
...
def test_g4_foreign_author_cannot_bind_others_packet() -> None:
    ...
    with pytest.raises(ZeroRelayReviewTaskError) as exc:
        bind_direct_review_route(_route_task(task), review=task,
                                 author=_identity(result_sha256="f" * 64))
...
def test_g4_result_destination_mismatch_rejected() -> None:
    ...
    with pytest.raises(ZeroRelayReviewTaskError) as exc:
        bind_direct_review_route(wrong_dest, review=task, author=_identity())

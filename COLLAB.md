...
| `WO-P1-162` GLM-first execution / universal entry (renumbered from WO-P1-157; GPT2/#210 keeps WO157/AIP-3) | GLM-1 | ACTIVE / RECONCILED_ON_MAIN 2026-09-07 | Docs/process-only successor recomposed onto `main@df5a25f` with the PR #220 stacked repair folded (`STACKED_REPAIR_SEMANTICS = PASS`, CI `33947807332`); folds PR221/ZRA-1 merged truth and repairs the MERGED_NOT_FOLDED drift. Universal entry, task-selective project graph, capability-first routing, Zero-Relay transport-only boundary. Awaiting focused GPT1 rereview + exact-head CI + GPT-A merge; no product source/tests/runtime/DB/credentials. |

...
## 12. Models & Agents — Sunday Family provider surface (2026-08-28)

The multi-model/sub-agent accelerator is part of the existing A-Sunday Conductor command center. Do not create a separate vendor-branded dashboard.

- Settings/Advanced gains a compact `MODELS & AGENTS` section.
- `LEAD / ORCHESTRATOR` and `SUB-AGENT PROVIDERS` are roles; vendor/model names are secondary metadata.
- The first provider may display GLM-5.3 through a Claude Code/Anthropic-style harness, but visual hierarchy remains Sunday Family first.
- Provider rows may show health, model, harness, effort, trust class, quota/reset observation, and concurrency when the values are real and fresh.
- `CONFIGURED` must never be rendered as `READY` without observed health evidence.
- Secrets are always masked; full credential values must never be placed in labels, logs, tooltips, copy actions, screenshots generated by the application, or diagnostic exports.
- Third-party proxy trust/data-egress status must be inspectable; a healthy provider may still be policy-ineligible.
- Main-screen clutter stays low. Detailed endpoint/credential/quota configuration belongs in Settings/Advanced, while the main execution view may show only the selected provider and reason.
- Preserve the current dark near-black, fine-border, compact monospaced, restrained-status-accent visual language and the Sunday Family particle identity.
